...
function getNode($answer){
	...
	while ($rows = $answer[0]->fetch()) {
		...
		if (!array_key_exists("bureau",$rows))
			$rows['bureau']="";
		if (!array_key_exists("telephone",$rows))
			$rows['telephone']="";
		if (!array_key_exists("profession",$rows))
			$rows['profession']="";
		$json['id'] = utf8_encode ($rows['id'] .substr($rows['name'], 0, 4) );//. substr($rows['name'], -5, 2);
		$json['selected'] = 'false';
		$json['name'] = $rows['name'];
		if (array_key_exists("description",$rows))
...
function getEdges($answer){
	...
	while ($rows = $answer[0]->fetch()) {
		...
		if (!array_key_exists("last_name",$rows))	
			$rows['last_name']="";
